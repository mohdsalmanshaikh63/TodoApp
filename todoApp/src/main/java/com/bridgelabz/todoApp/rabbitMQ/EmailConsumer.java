...
public class EmailConsumer implements MessageListener {
	...
	public void onMessage(Message message) {
		...
		System.out.println("************Message body is " + new String(message.getBody()));

		System.out.println("************Mail Utility is " + mailUtility);

		Gson gson = new GsonBuilder().create();

		// Email email = jsonHelper.convertFromJSON(message.getBody().toString());

		Email email = gson.fromJson(new String(message.getBody()), Email.class);
